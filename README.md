<p align="right">
  <a href="#english-version">
    <img src="https://img.shields.io/badge/English-Version-0E7490?style=flat-square" />
  </a>
  <a href="#中文版本">
    <img src="https://img.shields.io/badge/中文版本-EA4335?style=flat-square" />
  </a>
</p>

<a id="english-version"></a>

<div align="center">

# 🧭 Yuliang Wang

### Maritime Studies · Shipping Data Analytics · Port & Supply Chain

<p>
  <a href="https://love-ovo.github.io/">
    <img src="https://img.shields.io/badge/Website-love--ovo.github.io-0E7490?style=for-the-badge&logo=githubpages&logoColor=white" />
  </a>
  <a href="mailto:yuliangwang629@gmail.com">
    <img src="https://img.shields.io/badge/Email-yuliangwang629%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/yuliang-wang-b16153350">
    <img src="https://img.shields.io/badge/LinkedIn-Yuliang%20Wang-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://orcid.org/0009-0007-7998-3853">
    <img src="https://img.shields.io/badge/ORCID-0009--0007--7998--3853-A6CE39?style=for-the-badge&logo=orcid&logoColor=white" />
  </a>
</p>

<p>
  <img src="https://readme-typing-svg.demolab.com?font=Source+Sans+3&size=22&duration=3500&pause=900&color=0E7490&center=true&vCenter=true&width=760&lines=Turning+maritime+complexity+into+data-informed+decisions;Exploring+shipping+data%2C+port+operations%2C+and+supply+chains;Connecting+maritime+knowledge+with+operational+insight" />
</p>

</div>

---

## 🌊 About This Portfolio

This repository hosts my personal academic and professional homepage:

### 🔗 [Visit my website → love-ovo.github.io](https://love-ovo.github.io/)

Hi, I'm **Yuliang Wang**, a Master's student in **Maritime Studies** at **Nanyang Technological University**.

My background combines **maritime operations**, **freight forwarding**, **shipping data analysis**, **logistics**, and **supply chain research**. This portfolio presents my academic background, research interests, selected publications, technical skills, and professional direction.

> **Maritime knowledge + data analytics + operational insight**

---

## 🛰️ Portfolio Mission Control

| Destination | Description | Link |
|---|---|---|
| 🌐 **Personal Website** | Full academic and professional portfolio | [Open Website](https://love-ovo.github.io/) |
| 📄 **CV & Documents** | CV and supporting documents page | [Open CV Page](https://love-ovo.github.io/cv%26ps.html) |
| 🚢 **Projects & Research** | Maritime, logistics, and data-related research highlights | [View Projects](https://love-ovo.github.io/#projects) |
| 📚 **Publications** | Selected academic publications and research outputs | [View Publications](https://love-ovo.github.io/#publications) |
| 📬 **Contact** | Email, LinkedIn, GitHub, and ORCID | [Contact Me](https://love-ovo.github.io/#contact) |

---

## 🧭 Research Atlas

<table>
  <tr>
    <td width="50%">
      <h3>🚢 Maritime Operations</h3>
      <p>
        Understanding shipping processes, freight forwarding, port operations, and maritime safety from an operational perspective.
      </p>
    </td>
    <td width="50%">
      <h3>📊 Shipping Data Analytics</h3>
      <p>
        Applying Python, MATLAB, SPSS, GIS, and forecasting methods to transform maritime and logistics data into clearer insights.
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3>🏗️ Port & Supply Chain</h3>
      <p>
        Exploring cargo throughput, logistics networks, port planning, and supply chain efficiency in maritime transportation systems.
      </p>
    </td>
    <td width="50%">
      <h3>🌱 Sustainable Decisions</h3>
      <p>
        Connecting maritime knowledge with data-informed decision-making for more efficient, resilient, and sustainable operations.
      </p>
    </td>
  </tr>
</table>

---

## 📊 Data-to-Decision Workflow

| Stage | Focus | Output |
|---|---|---|
| **01 · Maritime Context** | Shipping, ports, logistics, safety, and operational problems | Clear research or operational question |
| **02 · Data Collection** | Maritime, port, logistics, or transportation-related data | Structured data source |
| **03 · Processing & Analysis** | Python, MATLAB, SPSS, GIS, Excel, and statistical methods | Cleaned and interpretable dataset |
| **04 · Modeling & Forecasting** | Forecasting, optimization, and quantitative models | Analytical results |
| **05 · Operational Insight** | Translating analysis into recommendations | Port, shipping, or supply chain decisions |

---

## 🧩 Core Focus

<p>
  <img src="https://img.shields.io/badge/Maritime%20Studies-0B2545?style=flat-square" />
  <img src="https://img.shields.io/badge/Shipping%20Data%20Analytics-0E7490?style=flat-square" />
  <img src="https://img.shields.io/badge/Port%20Operations-0369A1?style=flat-square" />
  <img src="https://img.shields.io/badge/Supply%20Chain%20Research-155E75?style=flat-square" />
  <img src="https://img.shields.io/badge/Maritime%20Safety-164E63?style=flat-square" />
  <img src="https://img.shields.io/badge/Freight%20Forwarding-0F766E?style=flat-square" />
  <img src="https://img.shields.io/badge/Transportation-075985?style=flat-square" />
</p>

<table>
  <tr>
    <td width="200"><strong>🚢&nbsp;Domain&nbsp;Knowledge</strong></td>
    <td>Maritime Studies · Shipping Operations · Freight Forwarding · Maritime Safety</td>
  </tr>
  <tr>
    <td><strong>📊 Analytical Tools</strong></td>
    <td>Python · MATLAB · SPSS · GIS · Forecasting · Data Visualization</td>
  </tr>
  <tr>
    <td><strong>🏗️ Research Direction</strong></td>
    <td>Port Operations · Cargo Throughput Forecasting · Logistics · Supply Chain Research</td>
  </tr>
  <tr>
    <td><strong>🌐 Professional Skills</strong></td>
    <td>Research Writing · Project Coordination · Cross-functional Communication · Presentations</td>
  </tr>
</table>

---

## 📚 Selected Publications

| Type | Publication |
|---|---|
| **Journal** | **Wang, Y.** Prediction of Shenzhen Port cargo throughput based on the nonlinear optimized GM-Markov model. *China Water Transport*, 25(3), 15–17. |
| **Journal** | **Wang, Y.** Research on inland river crew team building under the new development pattern: Analysis based on Porter's five forces model. *Pearl River Water Transport*, 2024(10), 101–103. |
| **Forecasting** | **Wang, Y.** Improved GM-Markov model based on CPSO algorithm for port cargo throughput forecasting. *Journal of Chongqing Jiaotong University (Natural Sciences)*. |
| **Conference** | **Wang, Y.** Large-Scale Sampling Model and Optimization Algorithm Based on Dynamic Programming and Monte Carlo Simulation. *The 5th International Conference on Multi-modal Information Analytics*. |

---

## 🛠️ Tech Stack

<p>
  <img src="https://img.shields.io/badge/HTML5-Structure-E34F26?style=flat-square&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-Styling-1572B6?style=flat-square&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-Interaction-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/GitHub%20Pages-Deployment-222222?style=flat-square&logo=githubpages&logoColor=white" />
</p>

<p>
  <img src="https://img.shields.io/badge/Python-Analytics-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/MATLAB-Modeling-0076A8?style=flat-square" />
  <img src="https://img.shields.io/badge/SPSS-Statistics-1F70C1?style=flat-square" />
  <img src="https://img.shields.io/badge/GIS-Spatial%20Analysis-2E7D32?style=flat-square" />
  <img src="https://img.shields.io/badge/Excel-Data%20Processing-217346?style=flat-square&logo=microsoftexcel&logoColor=white" />
</p>

---

## 🧱 Website Architecture

<details>
<summary><strong>Click to view repository structure</strong></summary>

```text
love-ovo.github.io/
├── index.html          # Homepage
├── cv&ps.html          # CV and documents page
├── style.css           # Main styling
├── script.js           # Theme toggle and page interactions
├── assets/             # CV and PDF documents
└── images/             # Profile and project images
```

</details>

---

## 🎨 Design Direction

The visual identity of this website is shaped around:

- **Maritime blue tones**
- **Clean academic layout**
- **Data-driven professional presentation**
- **Readable sections with strong spacing**
- **A balance between research profile and operational portfolio**

---

## 📬 Contact

<p>
  <a href="mailto:yuliangwang629@gmail.com">
    <img src="https://img.shields.io/badge/Email-yuliangwang629%40gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/love-ovo">
    <img src="https://img.shields.io/badge/GitHub-love--ovo-181717?style=flat-square&logo=github&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/yuliang-wang-b16153350">
    <img src="https://img.shields.io/badge/LinkedIn-Yuliang%20Wang-0A66C2?style=flat-square&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://orcid.org/0009-0007-7998-3853">
    <img src="https://img.shields.io/badge/ORCID-0009--0007--7998--3853-A6CE39?style=flat-square&logo=orcid&logoColor=white" />
  </a>
</p>

---

<div align="center">

### ⚓ Maritime knowledge · Data analytics · Operational insight

</div>

---

<p align="right">
  <a href="#english-version">
    <img src="https://img.shields.io/badge/English-Version-0E7490?style=flat-square" />
  </a>
  <a href="#中文版本">
    <img src="https://img.shields.io/badge/中文版本-EA4335?style=flat-square" />
  </a>
</p>

<a id="中文版本"></a>

<div align="center">

# 🧭 王俞亮

### 海事研究 · 航运数据分析 · 港口与供应链

<p>
  <a href="https://love-ovo.github.io/">
    <img src="https://img.shields.io/badge/Website-love--ovo.github.io-0E7490?style=for-the-badge&logo=githubpages&logoColor=white" />
  </a>
  <a href="mailto:yuliangwang629@gmail.com">
    <img src="https://img.shields.io/badge/Email-yuliangwang629%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/yuliang-wang-b16153350">
    <img src="https://img.shields.io/badge/LinkedIn-Yuliang%20Wang-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://orcid.org/0009-0007-7998-3853">
    <img src="https://img.shields.io/badge/ORCID-0009--0007--7998--3853-A6CE39?style=for-the-badge&logo=orcid&logoColor=white" />
  </a>
</p>

<p>
  <img src="https://readme-typing-svg.demolab.com?font=Source+Sans+3&size=22&duration=3500&pause=900&color=0E7490&center=true&vCenter=true&width=760&lines=%E5%B0%86%E6%B5%B7%E4%BA%8B%E5%A4%8D%E6%9D%82%E9%97%AE%E9%A2%98%E8%BD%AC%E5%8C%96%E4%B8%BA%E6%95%B0%E6%8D%AE%E9%A9%B1%E5%8A%A8%E7%9A%84%E5%86%B3%E7%AD%96;%E6%8E%A2%E7%B4%A2%E8%88%AA%E8%BF%90%E6%95%B0%E6%8D%AE%2C+%E6%B8%AF%E5%8F%A3%E8%BF%90%E8%90%A5%E4%B8%8E%E4%BE%9B%E5%BA%94%E9%93%BE;%E8%BF%9E%E6%8E%A5%E6%B5%B7%E4%BA%8B%E7%9F%A5%E8%AF%86%E4%B8%8E%E8%BF%90%E8%90%A5%E6%B4%9E%E5%AF%9F" />
</p>

</div>

---

## 🌊 关于这个作品集

这个仓库用于展示我的个人学术与职业主页：

### 🔗 [访问我的网站 → love-ovo.github.io](https://love-ovo.github.io/)

你好，我是 **王俞亮**，目前是 **南洋理工大学 Maritime Studies** 硕士学生。

我的背景结合了 **海事运营**、**货运代理**、**航运数据分析**、**物流** 和 **供应链研究**。这个作品集展示了我的教育背景、研究兴趣、代表性发表、技术能力和职业方向。

> **海事知识 + 数据分析 + 运营洞察**

---

## 🛰️ 作品集导航

| 目的地 | 说明 | 链接 |
|---|---|---|
| 🌐 **个人网站** | 完整的学术与职业作品集 | [打开网站](https://love-ovo.github.io/) |
| 📄 **简历与文档** | 简历和支持材料页面 | [打开简历页面](https://love-ovo.github.io/cv%26ps.html) |
| 🚢 **项目与研究** | 海事、物流与数据相关研究亮点 | [查看项目](https://love-ovo.github.io/#projects) |
| 📚 **论文发表** | 代表性学术发表与研究成果 | [查看发表](https://love-ovo.github.io/#publications) |
| 📬 **联系方式** | 邮箱、LinkedIn、GitHub 和 ORCID | [联系我](https://love-ovo.github.io/#contact) |

---

## 🧭 研究方向地图

<table>
  <tr>
    <td width="50%">
      <h3>🚢 海事运营</h3>
      <p>
        从运营视角理解航运流程、货运代理、港口运营与海事安全。
      </p>
    </td>
    <td width="50%">
      <h3>📊 航运数据分析</h3>
      <p>
        使用 Python、MATLAB、SPSS、GIS 和预测方法，将海事与物流数据转化为清晰的分析洞察。
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3>🏗️ 港口与供应链</h3>
      <p>
        研究货物吞吐量、物流网络、港口规划和海上交通系统中的供应链效率。
      </p>
    </td>
    <td width="50%">
      <h3>🌱 可持续决策</h3>
      <p>
        将海事知识与数据驱动的决策方法结合，推动更高效、更具韧性和更可持续的运营。
      </p>
    </td>
  </tr>
</table>

---

## 📊 从数据到决策的流程

| 阶段 | 重点 | 输出 |
|---|---|---|
| **01 · 海事情境** | 航运、港口、物流、安全和运营问题 | 明确的研究或运营问题 |
| **02 · 数据收集** | 海事、港口、物流或交通相关数据 | 结构化数据来源 |
| **03 · 处理与分析** | Python、MATLAB、SPSS、GIS、Excel 和统计方法 | 清洗后且可解释的数据集 |
| **04 · 建模与预测** | 预测、优化和定量模型 | 分析结果 |
| **05 · 运营洞察** | 将分析转化为建议 | 港口、航运或供应链决策 |

---

## 🧩 核心方向

<p>
  <img src="https://img.shields.io/badge/Maritime%20Studies-0B2545?style=flat-square" />
  <img src="https://img.shields.io/badge/Shipping%20Data%20Analytics-0E7490?style=flat-square" />
  <img src="https://img.shields.io/badge/Port%20Operations-0369A1?style=flat-square" />
  <img src="https://img.shields.io/badge/Supply%20Chain%20Research-155E75?style=flat-square" />
  <img src="https://img.shields.io/badge/Maritime%20Safety-164E63?style=flat-square" />
  <img src="https://img.shields.io/badge/Freight%20Forwarding-0F766E?style=flat-square" />
  <img src="https://img.shields.io/badge/Transportation-075985?style=flat-square" />
</p>

<table>
  <tr>
    <td width="200"><strong>🚢&nbsp;领域知识</strong></td>
    <td>海事研究 · 航运运营 · 货运代理 · 海事安全</td>
  </tr>
  <tr>
    <td><strong>📊 分析工具</strong></td>
    <td>Python · MATLAB · SPSS · GIS · 预测建模 · 数据可视化</td>
  </tr>
  <tr>
    <td><strong>🏗️ 研究方向</strong></td>
    <td>港口运营 · 货物吞吐量预测 · 物流 · 供应链研究</td>
  </tr>
  <tr>
    <td><strong>🌐 职业能力</strong></td>
    <td>研究写作 · 项目协调 · 跨职能沟通 · 展示汇报</td>
  </tr>
</table>

---

## 📚 代表性发表

| 类型 | 发表 |
|---|---|
| **期刊** | **Wang, Y.** Prediction of Shenzhen Port cargo throughput based on the nonlinear optimized GM-Markov model. *China Water Transport*, 25(3), 15–17. |
| **期刊** | **Wang, Y.** Research on inland river crew team building under the new development pattern: Analysis based on Porter's five forces model. *Pearl River Water Transport*, 2024(10), 101–103. |
| **预测研究** | **Wang, Y.** Improved GM-Markov model based on CPSO algorithm for port cargo throughput forecasting. *Journal of Chongqing Jiaotong University (Natural Sciences)*. |
| **会议** | **Wang, Y.** Large-Scale Sampling Model and Optimization Algorithm Based on Dynamic Programming and Monte Carlo Simulation. *The 5th International Conference on Multi-modal Information Analytics*. |

---

## 🛠️ 技术栈

<p>
  <img src="https://img.shields.io/badge/HTML5-Structure-E34F26?style=flat-square&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-Styling-1572B6?style=flat-square&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-Interaction-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/GitHub%20Pages-Deployment-222222?style=flat-square&logo=githubpages&logoColor=white" />
</p>

<p>
  <img src="https://img.shields.io/badge/Python-Analytics-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/MATLAB-Modeling-0076A8?style=flat-square" />
  <img src="https://img.shields.io/badge/SPSS-Statistics-1F70C1?style=flat-square" />
  <img src="https://img.shields.io/badge/GIS-Spatial%20Analysis-2E7D32?style=flat-square" />
  <img src="https://img.shields.io/badge/Excel-Data%20Processing-217346?style=flat-square&logo=microsoftexcel&logoColor=white" />
</p>

---

## 🧱 网站结构

<details>
<summary><strong>点击查看仓库结构</strong></summary>

```text
love-ovo.github.io/
├── index.html          # 主页
├── cv&ps.html          # 简历和文档页面
├── style.css           # 主样式文件
├── script.js           # 主题切换和页面交互
├── assets/             # 简历和 PDF 文档
└── images/             # 个人照片和项目图片
```

</details>

---

## 🎨 设计方向

这个网站的视觉风格围绕以下方向：

- **海事蓝色调**
- **清晰的学术式布局**
- **数据驱动的职业展示**
- **具有良好间距的可读性分区**
- **研究画像与运营型作品集之间的平衡**

---

## 📬 联系方式

<p>
  <a href="mailto:yuliangwang629@gmail.com">
    <img src="https://img.shields.io/badge/Email-yuliangwang629%40gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/love-ovo">
    <img src="https://img.shields.io/badge/GitHub-love--ovo-181717?style=flat-square&logo=github&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/yuliang-wang-b16153350">
    <img src="https://img.shields.io/badge/LinkedIn-Yuliang%20Wang-0A66C2?style=flat-square&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://orcid.org/0009-0007-7998-3853">
    <img src="https://img.shields.io/badge/ORCID-0009--0007--7998--3853-A6CE39?style=flat-square&logo=orcid&logoColor=white" />
  </a>
</p>

---

<div align="center">

### ⚓ 海事知识 · 数据分析 · 运营洞察

</div>

<p align="right">
  <a href="#english-version">
    <img src="https://img.shields.io/badge/Back%20to-English-0E7490?style=flat-square" />
  </a>
</p>
